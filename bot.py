...
class Context:
    ...
    def _serialize_move(self, move: BasicMove) -> dict:
        d: dict[str, object] = {"command": move.command.name}
...
class AbstractMCTSBot(BaseAI):
    ...
    def play(self, game_state: GameState, possible_moves: List[BasicMove], remaining_time: int) -> BasicMove:
        ...
        try:
            move: BasicMove = self._play(game_state, possible_moves, remaining_time)
        except Exception:
            import traceback
            traceback.print_exc()
...
class FakeSaccarinaBot(AbstractMCTSBot):
    max_depth = 20
    move_timeout = 0.8
    turn_timeout = 10.
    ...
    def _run(self, tree: Search, depth: int) -> float:
        ...
        if tree.leaf:
            ...
            if tree.children[move] is None:
                tree.expand(move)
            # pyrefly: ignore [bad-argument-type]
            value = self._run(tree.children[move], depth+1)
        tree.update(value)
        return value

class FakeBestMCTS3(AbstractMCTSBot):
    turnTimeout = 10.
    moveTimeout = 0.8
    repeats = 20
    ...
    def _run(self, node: Search, num_moves: int)-> Tuple[float, int]:
        ...
        # pyrefly: ignore [bad-argument-type]
        result, num_moves = self._run(node.children[max_move], num_moves)
    ...
    def _play(self, game_state: GameState, possible_moves: List[BasicMove], remaining_time: int) -> BasicMove:
        ...
        for i in range(self.repeats):
            _, single_num_moves = self._run(self.tree, num_moves)
